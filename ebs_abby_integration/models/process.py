...
def setup_processor():
	if "ABBYY_APPID" in os.environ:
		process.ApplicationId = os.environ["ABBYY_APPID"]

	if "ABBYY_PWD" in os.environ:
		process.Password = os.environ["ABBYY_PWD"]

	# Proxy settings
	if "http_proxy" in os.environ:
		proxy_string = os.environ["http_proxy"]
		print("Using http proxy at {}".format(proxy_string))
		process.Proxies["http"] = proxy_string

	if "https_proxy" in os.environ:
		proxy_string = os.environ["https_proxy"]
		print("Using https proxy at {}".format(proxy_string))
		process.Proxies["https"] = proxy_string


# Recognize a file at filePath and save result to resultFilePath
def recognize_file(img, result_file_path,server_url,application_id,password):
	print("Uploading..")
	settings = ProcessingSettings()
	task = process.process_image(img, settings,server_url,application_id,password)
	if task is None:
		print("Error")
		return
	if task.Status == "NotEnoughCredits":
		print("Not enough credits to process the document. Please add more pages to your application's account.")
		raise ValidationError("Not enough credits to process the document. Please add more pages to your application's account.")
		#return
	...
	while task.is_active():
		time.sleep(5)
		print(".")
		task = process.get_task_status(task,server_url)

	print("Status = {}".format(task.Status))

	if task.Status == "Completed":
		if task.DownloadUrl is not None:
			process.download_result(task, result_file_path)
...
def main(image,target_file,ServerUrl,ApplicationId,Password):
	ServerUrl= ServerUrl ;ApplicationId=ApplicationId; Password=Password
	global process  #//global to implement the setting at runtime

	process = AbbyyOnlineSdk()
	rec = process.AbbyOnlineSdk(ServerUrl,ApplicationId,Password)
	process.serverUrl = rec[0]
	...
	if image:
 		recognize_file(image, target_File,rec[0],rec[1],rec[2])
	else:
		#print("No such file: {}".format(source_File))
		print("there is no image sent")
